...
<Dialog open={this.state.openModal} className='ProductEdit__Modal'>
    ...
    <DialogContent className='Modal__Content'>
        ...
        onChange={(ev)=>{this.setState({imgTemp: ev.target.value})}}
        margin='dense'
        label='URL Imagen'
        type='text'
        fullWidth
    ...
    <DialogActions className='Modal__Actions'>
        <Button onClick={this.handleChangeImg} variant='contained' startIcon={<CheckIcon />} className='ButtonWallakeep ButtonWallakeep__Green'>
        Aceptar
        </Button>
